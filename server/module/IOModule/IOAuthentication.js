...
function checkTokenValid(socket, next) {
   try {
      console.log('abc');
      var decoded = null;
      if (socket.handshake.auth.token) decoded = jwt.verify(socket.handshake.auth.token, Configs.SECRET_KEY);
      if (socket.handshake.query.token) decoded = jwt.verify(socket.handshake.query.token, Configs.SECRET_KEY);
